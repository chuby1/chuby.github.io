...
### 2. [컨셉](#21)
### 3. [관련 이미지와 동영상](#22)
### 4. [대표 이미지 그리고 컨셉과 대표이미지 기반 작품 묘사](#23)
### 5. [SmallerWolrd의 구성 요소](#24)
### 6. [게임 시스템 디자인](#25)
#####   [·   게임 오브젝트 분해](#26)
#####   [·   파라미터(속성)](#27)
#####   [·   행동](#28)
...
### 7. [개발 요구사항 & 흐름도](#32)
##### [·   요구사항(1년 / 6주)](#33)
##### [·   시간별 흐름도 flowchart[선택사항]](#34)
### 8. [스토리보드](#35)
### 9. [1주차 작업 결과](files/w01/index.md)
# [ 게임명 ] <a name='20'></a>
이 게임의 이름은 SmallerWolrd입니다.
<br><br>
# [ 컨셉 ] <a name='21'></a>

## 메인컨셉 : 도전
계속해서 원래 세계로 돌아가기 위해 도전하는 플레이어를 표현했습니다.


### 서브 컨셉 1 : 다양성
여러 상호작용을 이용하여 스테이지 구성 및 다양한 적 등장
<br><br>

### 서브 컨셉 2 : 입체감
3d시점을 통한 시각적인 부분에 대해 플레이어가 현실감을 느끼게함
<br><br>
...
### 서브 컨셉 4 : 현실감
현실과 같지만 플레이어의 몸만 작아진 평행세계로 현실감 부여
<br><br>
...
# [관련 이미지 & 동영상] <a name='22'></a>
- 이미지  
   <img src="./img/SmallerWorld 게임화면.jpg" width="100%">
...
[이야기] : 평범하게 삶을 살아가고 있는 주인공이 갑지기 자신의 몸만 작아진 상태로 다른 평행세계에 온 경우입니다.

[만들게 된 배경]  

평소에 어드벤처 게임을 자주 하여 이러한 게임을 만들어 보고 싶었습니다.

[카메라 관점]  

현실에서 작아진 플레이어를 잘 느낄 수 있게 3인칭 이지만 1인칭에 가깝게 시점을 잡았습니다.
<br>
...
수풀 맵 : 미로에 숨겨진 적과 아이템을 찾아서 탈출하는 맵입니다.

하우스 맵 : 미로에서 문을 열 수 있는 키를 찾아 문을 열고 원래 세계로 돌아가기 위해 방을 돌아다니며 미션을 깨게  됩니다.
...
1.게임 배경으로 플레이어가 어떤 게임인지 인지

2.게임 기본 설정 설정과 게임 시작을 할수 있는 로비 

3.여러 적과 플레이어의 상호작용 및 미로 파악

3.미로 탈출 이후 집에서 미션 깨기 
## 4. 기술

기본적으로 유니티 엔진을 기반으로 구현할 예정이고, 구현하기 힘든 이미지나 오브젝트는 에셋스토어 또는 외부에서 가져와 사용할 예정입니다.


# [게임 시스템 디자인] <a name='25'></a>
1. 게임 오브젝트 분해 (구성 요소 분석) <a name='26'></a>
...
|:----:|:------------:|:-----------------------------------:|:---:|
|1|점프|TryJump|플레이어 점프||
|2|이동|Move|플레이어 이동 (wasd로 이동 가능)||
|3|카메라 위치|CameraRotation|플레이어 카메라 위치 설정||
|4|캐릭터 위치|CharacterRotation|플레이어 위치 설정||
|5|걷기|TryWalk|플레이어 걷기(기본 속도로 걷는다)||
<br><br>
...
|:----:|:------------:|:-----------------------------------:|:---:|
|1|충돌|HitCoroutine|손과 다른 오브젝트 충동작용||
|2|확인|CheckObject|다른 오브젝트 상세 확인||
|3|놓기|TryDrop|플레이어 놓기 (플레이어 바로 앞에 놓는다.)||
|4|줍기|TryKeep|플레이어 줍기(물건은 한가지만 들고있을수 있다)||
<br><br>
...
|속성|영문명칭|설명|비고|
|:----:|:------------:|:-----------------------------------:|:---:|
|풀숲 스테이지|CaveStage|첫 번째 스테이지, 풀숲에 미로가 있고 이 미로에는 벌레 모양의 적이 있다.||
|집 스테이지|HouseStage|두 번째 스테이지, 여러 미션이 있으면 클리어시 원래 세계로 갈 수 있다.||
...
|앞으로 이동 (w)|플레이어가 일정 속도로 앞으로 간다.|
|뒤로 이동 (s)|플레이어가 뒤로 이동|
|줍기 (e)|나오는 오브젝트 줍기.(인벤토리 창으로 확인가능)|
|설정 창(esc)|게임 정지 및 여러 설정 가능|
|문열기 (마우스 좌클릭)|문 및 통로 개방|
<br><br>

플레이어 캐릭터 속성(파라미터) <a name='29'></a>
...
4.게임의 규칙 <a name='15'></a>
 한명의 플레이어가 2개의 스테이지를 클리어해 엔딩에 도달한다.

5.게임에서 사용될 공식 <a name='31'></a>
Unity 3D
# [게임 시나리오] <a name='35'></a>
게임 시나리오(개발자)
1788017 성시엽
...
1. 조작방법
PC로 조작하는 게임임으로 기본적으로 이동은 WASD로 이뤄지고, 점프 SPACE , 줍기E, 상호 작용하기 마우스 좌클릭을 사용한다. 조사가 가능한 물체가 앞에 있으면 마우스 좌클릭을 통해 상호작용을 하거나 문제 풀이에 용이하게 확대된다.

<br><br>
2. 타이틀 화면

- 이미지  
  <img src="./img/SmallerWorld 메인화면.jpg" width="50%">
  
게임을 시작하면 게임 시작 , 환경설정, 그리고 게임종료로 세가지 메뉴가 보인다. 게임하기 버튼을 누르면 게임을 시작한다. 
설정 버튼의 경우 인게임 내에 BGM 소리 설정을 넣을 예정이다.
<br><br>

3. 인게임 화면 
- 이미지  
  <img src="./img/SmallerWorld 게임화면.jpg" width="50%">
  

<br><br>

4. 화면이동
시점의 경우 3인칭 시점으로 키보드(wasd)와 마우스(시점의 이동)를 통해 이뤄진다.
<br><br>
...
<br><br>
# [게임 시나리오(플레이어)] <a name='36'></a>

<br><br>
# [ 개발 요구사항 및 흐름도] <a name='32'></a>
요구사항
1. 시작화면 , 인벤토리 화면, 게임화면 총 3개의 화면이 있다.
2. 시작화면에는 시작하기와 설정 버튼, 게임종료가 있다.
3. 설정버튼은 소리를 설정할 수 있다.
